...
app.controller('itemCatController', function($scope, $controller,
	...
	// 根据上级 ID 查询列表
	$scope.findByParentId = function(parantId) {
		
		$scope.parentId = parantId;//记住上级ID
		
		itemCatService.findByParentId(parantId).success(
				function(response) {
			$scope.list = response;
		});
	}
	...
	$scope.selectList=function(p_entity){
		if($scope.grade==1){//如果级别为1级
			$scope.entity_1=null;
			$scope.entity_2=null;
		}
		if($scope.grade==2){//如果级别为2级
			$scope.entity_1=p_entity;
			$scope.entity_2=null;
		}
		if($scope.grade==3){//如果级别为3级	
